function varargout=filter_JA_modified(trj,lambda, tt_in, tt_out, endpoints,vel,acc,direction,method)
...
for di=1:d
    ...
    F1 = griddedInterpolant(tt_in(~isnan(trj(:,di))),trj(~isnan(trj(:,di)),di),'cubic');
    
    %%%
    if isempty(endpoints)
        rx0=F1(tt_in(1)); rx1=F1(tt_in(end));
    else
    ...
    guessx2=bvpinit(tt_in,guess_funx);
    ...
    varargout{di} = deval(solx,tt_out)';
    varargout{di + d} = solx;
end
