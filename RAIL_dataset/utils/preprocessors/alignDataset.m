...
traj_source = [dataset(ind_source).time, dataset(ind_source).pos(:,1:3)];

dataset_warped = struct([]);
for ii = 1:length(dataset)
    
    traj_target = [dataset(ii).time, dataset(ii).pos(:,1:3)];
    ...
    dataset_warped(ii,1).time = Demo.time;
    dataset_warped(ii,1).pos = Demo.pos;
